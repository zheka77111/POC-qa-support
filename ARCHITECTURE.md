...
Ядром является langchain react агент. Такой подход позволяет в дальнейшем подключать различные модули в виде skills, долгосрочную память и поддержку работы с файлами, различные планировщики и подрезка контекста и др.
В качестве LLM используется GigaChat, возможно подключить другие.
Остальная функциональность основана на логике langgraph с параллельной обработкой, условными переходами.
